...
class VideoProcessor:
    ...
    def _analyze_content(self, job, transcript_data):
        ...
        try:
            ...
            for segment in segments:
                ...
                # Get duration preferences from job
                min_duration, max_duration = self._get_duration_range(job.short_duration)
                duration = segment.end_time - segment.start_time

                # Lower the threshold and be more flexible with duration
                if (segment.overall_score > 0.2 and  # Much lower threshold
                    min_duration <= duration <= max_duration and
                    len(segment.text.split()) >= 3):  # Lower word count
            ...
            if len(engaging_segments) < job.max_shorts:
                ...
                for segment in all_segments:
                    if segment not in engaging_segments:
                        duration = segment.end_time - segment.start_time
                        if min_duration <= duration <= max_duration and len(segment.text.split()) >= 2:
                            segment.overall_score = max(0.2, segment.overall_score)  # Ensure minimum score
                            engaging_segments.append(segment)
                            if len(engaging_segments) >= job.max_shorts:
                                break

            # If still not enough, create overlapping segments
            if len(engaging_segments) < job.max_shorts:
                self._create_additional_segments(job, engaging_segments)

            return engaging_segments[:job.max_shorts]  # Return user-specified number of segments
            ...
            for segment in segments:
                ...
                if min_duration <= duration <= max_duration:
                    segment.overall_score = 0.3  # Default score
                    fallback_segments.append(segment)
                    if len(fallback_segments) >= job.max_shorts:
                        break
            
            return fallback_segments[:job.max_shorts]
            ...
            for i, segment in enumerate(engaging_segments):
                try:
                    ...
                    metadata = self.gemini_analyzer.generate_metadata(
                        segment_text, 
                        job.title or "YouTube Video",
                        job.content_language or 'hinglish'
                    )
                    ...
                    # Generate thumbnail
                    self._generate_thumbnail(output_path, thumbnail_path, job.aspect_ratio)
    ...
    def _create_vertical_video(self, input_path, output_path, start_time, end_time, aspect_ratio='9:16'):
        """Create vertical video from horizontal source using FFmpeg"""
        try:
            ...
            if aspect_ratio == '1:1':
                ...
                scale_filter = 'scale=1080:1080:force_original_aspect_ratio=increase,crop=1080:1080'
            else:  # Default to 9:16
                # Vertical format (1080x1920)
                scale_filter = 'scale=1080:1920:force_original_aspect_ratio=increase,crop=1080:1920'

            # FFmpeg command to create video with specified aspect ratio
            cmd = [
                'ffmpeg',
                '-i', input_path,
                '-ss', str(start_time),
    ...
    def _generate_thumbnail(self, video_path, thumbnail_path, aspect_ratio='9:16'):
        """Generate thumbnail from video"""
        try:
            # Configure thumbnail dimensions based on aspect ratio
            if aspect_ratio == '1:1':
                # Square thumbnail (640x640)
                thumbnail_size = '640x640'
            else:  # Default to 9:16
                # Vertical thumbnail (640x1136)
                thumbnail_size = '640x1136'
    ...
    def _cleanup_temporary_files(self, job):
        ...
        try:
            ...
            # Clean up any other temporary files in temp directory for this job
            temp_dir = 'temp'
            if os.path.exists(temp_dir):
                for filename in os.listdir(temp_dir):
                    if f'_{job.id}' in filename:
            ...
            self.logger.warning(f"Error during temporary file cleanup for job {job.id}: {e}")

    def _create_additional_segments(self, job, existing_segments):
        """Create additional segments by splitting longer segments or using overlapping segments"""
        try:
            min_duration, max_duration = self._get_duration_range(job.short_duration)
            segments_needed = job.max_shorts - len(existing_segments)
            
            if segments_needed <= 0:
                return
                
            # Get video duration to create additional segments
            all_segments = TranscriptSegment.query.filter_by(job_id=job.id).all()
            if all_segments:
                total_duration = max(seg.end_time for seg in all_segments)
                
                # Create overlapping segments with the target duration
                segment_duration = (min_duration + max_duration) / 2  # Use average duration
                overlap = segment_duration * 0.3  # 30% overlap
                
                current_start = 0
                created_count = 0
                
                while current_start < total_duration and created_count < segments_needed:
                    ...
                    for existing_seg in existing_segments:
                        if (current_start < existing_seg.end_time and segment_end > existing_seg.start_time):
                            overlap_ratio = (min(segment_end, existing_seg.end_time) - max(current_start, existing_seg.start_time)) / segment_duration
                            if overlap_ratio > 0.7:  # More than 70% overlap
                                overlap_exists = True
                                break
                    
                    if not overlap_exists and segment_end - current_start >= min_duration:
                        # Create new segment
                        new_segment = TranscriptSegment()
                        new_segment.job_id = job.id
                        new_segment.start_time = current_start
                        new_segment.end_time = segment_end
                        new_segment.text = f"Additional segment from {current_start:.1f}s to {segment_end:.1f}s"
                        new_segment.overall_score = 0.25  # Lower score for additional segments
                        new_segment.engagement_score = 0.25
                        new_segment.emotion_score = 0.25
                        new_segment.viral_potential = 0.25
                        new_segment.quotability = 0.25
                        new_segment.emotions_detected = []
                        new_segment.keywords = []
                        new_segment.analysis_notes = "Additional segment created to meet requested count"
                ...
                db.session.commit()
                self.logger.info(f"Created {created_count} additional segments for job {job.id}")
                
        except Exception as e:
            self.logger.error(f"Failed to create additional segments: {e}")

    def _get_duration_range(self, short_duration):
        """Get min and max duration based on user preference"""
        duration_ranges = {
            '15-30': (15, 30),
            '30-45': (30, 45),
            '45-60': (45, 60),
        ...
        return duration_ranges.get(short_duration, (30, 45))